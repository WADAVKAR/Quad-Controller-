...
for qn = 1:nquad
    ...
    h_euler{qn} = figure('Name', ['Quad ' num2str(qn) ' : attitude']);
    plot_state(h_euler{qn}, QP{qn}.state_hist(7:10,:), QP{qn}.time_hist, 'euler', 'vic');
    plot_state(h_euler{qn}, QP{qn}.state_des_hist(7:10,:), QP{qn}.time_hist, 'euler', 'des');
end
if(~isempty(err))
    error(err);
end
